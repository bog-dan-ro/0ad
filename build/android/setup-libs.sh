...
if [ "$build_js" = "true" ]; then
  ...
  tar xvf ../../libraries/source/spidermonkey/mozjs-$MOZJS_VERSION.rc0.tar.bz2 -C temp/
  pushd temp/mozjs-$MOZJS_VERSION/js/src
  for f in ../../../../../../libraries/source/spidermonkey/*.diff; do
    patch -p3 < $f
  done
  patch -p3 < ../../../../jsshell.diff
  ./configure \
  ...
  make $JOBS || echo Never mind
  make install || echo Never mind
  # mozjs 31.2.0.rc0 compilation for android is completely screwed
  # we need to copy some files manulally
  cp -L dist/include/js-config.h $INSTALL_PREFIX/include/mozjs-31
  popd
fi
...
function compileAutoConf {
  ...
  ./configure --host=$TOOLCHAIN_PREFIX --with-sysroot=$SYSROOT --prefix=$INSTALL_PREFIX \
              ...
              --with-openssl=$INSTALL_PREFIX --with-ssl=$INSTALL_PREFIX --with-gnutls=no --without-lzma
  make $JOBS
  make install
  popd
}
...
cp -Lr $INSTALL_PREFIX $FINAL_INSTALL_PREFIX
rm -fr $INSTALL_PREFIX
rm -fr temp
popd
